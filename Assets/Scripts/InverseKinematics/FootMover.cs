...
public class FootMover : MonoBehaviour {
    ...
    private void Update() {
        ...
        if (Physics.Raycast(transform.position, -transform.up, out hit)) {
            if ((Vector3.Distance(hit.point, _targetPoint.position) >
                 _distance)) // Проверяем расстояние между попаданием рейкаста и текущим таргетом.
            {
                currentTime = 0;
            ...
            if (currentTime < 1) {
                Vector3 footPosition =
                    Vector3.Lerp(_targetPoint.position, hit.point,
                        _countLerpPosition); // С помощью линейной интерполяции плавненько переходим из текущей точки, в новую. 
                footPosition.y = Mathf.Lerp(footPosition.y, hit.point.y, _countLerpHeight) +
                                 (Mathf.Sin(currentTime * Mathf.PI) *
                                  _amplutide); // С помощью линейной интерполяции и синуса задаем новую высоту лапки. 
